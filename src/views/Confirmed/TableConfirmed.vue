...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      confirmedOrders() {
        let orders = this.allOrders.filter(ele => ele.statussuivi === 'Confirmed').map(ele => {
      ...
      filterByMonth() {
        let orders = ''
        if (this.date) {
          orders = this.confirmedOrders.filter(ele => {
      ...
      filterByDay() {
        ...
        if (this.date) {
          orders = this.confirmedOrders.filter(ele => {
            return parseInt(ele.Date.split('-')[1], 10) === parseInt(this.date.dateSelected.split('-')[1], 10) &&
              // month
